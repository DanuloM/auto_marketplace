...
class CarSerializer(serializers.ModelSerializer):
    brand = serializers.CharField(read_only=True, source="model.manufacturer.name")
    ...
    def validate(self, data):
        ...
        if model and generation and model != generation.model:
            raise serializers.ValidationError(
                "Generation does not belong to the selected model."
            )

        if year and generation:
            if generation.year_start and year < generation.year_start:
                raise serializers.ValidationError(
                    "Year is before generation start year."
                )
